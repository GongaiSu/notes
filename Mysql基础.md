...
![image-20211007173312237](/Volumes/生态/mysql/课件md版/01_课件/images/image-20211007173312237.png)
...
![image-20211012003508730](/Volumes/生态/mysql/课件md版/01_课件/images/image-20211012003508730.png)
...
![image-20211104192516324](/Volumes/生态/mysql/课件md版/01_课件/images/image-20211104192516324.png)
...
![image-20211104192912988](/Volumes/生态/mysql/课件md版/01_课件/images/image-20211104192912988.png)

- MultiPoint、MultiLineString、MultiPolygon、GeometryCollection 这4种类型都是集合类，是多个Point、LineString或Polygon组合而成。

下面展示的是多个同类或异类几何图形元素的组合：

![image-20211104193330204](/Volumes/生态/mysql/课件md版/01_课件/images/image-20211104193330204.png)
...
* Not null (非空约束)：设置字段不为空，‘‘和0不等于null。
  * 在创建表后添加非空约束：alter table 表名 modify 字段名 属性 not null; 
  * 在创建表后删除非空约束：alter table 表名 modify 字段名 属性  null; 
* Unique(唯一约束)：设置字段唯一，不能重复。
  * 在创建表后添加唯一约束
    * alter table 表名 modify 字段名 属性 unqiue; 
    * alter table 表名 add (constraint 约束名) unqiue(字段名);
  * 复合唯一约束
    * 在创建表的时候在字段的最后添加constraint 约束名 unqiue(字段一,字段二,…);
  * 删除唯一性约束：
    * alter table 表名 drop index 约束名
      * 新建或者添加约束时有约束名，删除则用约束名删除。
      * 没有约束名
        * 单列则是列名。
        * 复合列，这是第一个字段的列名。
* primary key(主键约束)
  * 在创建表后添加唯一约束:ALTER TABLE 表名 add primary key (字段名)
  * 在创建表后删除非空约束:alter table 表名 drop primary key;
* AUTO_INCREMENT(字段自增长)
  * 字段必须是主键或者有唯一约束，类型必须为整型。
  * 在创建表后添加字段自增长:ALTER TABLE 表名 modify  主键或者唯一 属性 auto_increment;
  * 在创建表后删除字段自增长:ALTER TABLE 表名 modify  主键或者唯一 属性
* Foreign key(外健约束)
  * constraint 外健名 foreign key (从表需要关联的字段) references (主表需要关联的字段)
  * 在创建表后添加外健约束:alter table 从表 add constraint 外健名 foreign key (从表需要关联的字段) references (主表需要关联的字段)
  * 在创建表后删除外健约束:alter table 从表 drop foreign key 外健名
* default(默认值)
  * 在创建表后添加字段默认值:ALTER TABLE 表名 modify  字段 属性 default;
  * 在创建表后删除字段默认值:ALTER TABLE 表名 modify  字段 属性;

 # 视图

## 创建视图

* create view 试图名 as select语句

## 查看视图

* show tables：查看表和视图
* Desc 视图名：查看视图的结构
* show table status like '视图名'：查看视图的属性信息
* show create view 视图名 ：查看视图的详细定义信息

## 更新视图

* 一般情况
  * 增删改和表的语句一样，修改视图对应的表的数据也会改变，修改表视图也会发生变化
* 特殊情况
  * 视图中的字段和表中的字段不能一一对应，视图中存在表中没有的字段，无法进行修改视图

## 修改视图

* create or replace view 视图名 as 查询语句：创建视图，如果视图存在，则更新使用的字段。
